...
public class VertxMeterBinderRestEasyReactiveFilter {
    ...
    public void filter(SimpleResourceInfo simpleResourceInfo, RoutingContext routingContext) {
        if (routingContext == null || routingContext.get(HttpRequestMetricInfo.HTTP_REQUEST_PATH_MATCHED) != null) {
            return;
        }
        routingContext.put(HttpRequestMetricInfo.HTTP_REQUEST_PATH, simpleResourceInfo.getTemplateUriPath());
    }
}
