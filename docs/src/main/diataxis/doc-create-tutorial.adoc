...
In this tutorial, we will create a new tutorial that guides users through creating, running, and testing a Quarkus application that uses annotations from an imaginary extension. We will work from the common tutorial template, describe a multi-step process, include code snippets, and explain what the user should expect to see at the end of each step.
...
+
It should reflect what the user will accomplish by completing it. The title should use sentence case and the active tense. Try answering the question: what will they do?
+
...
.Document Preview
--
The preview of your document should now include an Info callout box below the abstract explaining that the plugin is experimental technology, and describing what can be expected in terms of stability and/or support for experimental technologies.
--
...
Let's create a file called `acme-serve-http-requests-MyAcmeApplicaton.java`. While this is not a valid Java file name, it does sit nicely next to our tutorial source. We're going to rely on IDE magic for syntax checking.
...
- There are Asciidoc content regios defined by tag pairs (`tag::` and `end::`) in comments surrounding different sections of code.
...
Create a new source file, `src/main/java/org/acme/MyAcmeApplicaton.java`, and define the `MyAcmeApplicaton` bean as follows:

[source,java]
----
\include::{examples}/acme-serve-http-requests-MyAcmeApplicaton.java[tags=*;!goodbye]
----

<1> Specify the CDI lifecycle of this bean. An `@ApplicationScoped` bean has a single bean instance that is used for the application.
<2> The `@Anvil` annotation always implies a simple Http GET request with the uri derived from the method name, `/hello` in this case. The `optional` value indicates that the `name` parameter is not required.
-----
...
WARNING: Be careful with the amount of explanation given in a tutorial. Include enough information in the tutorial to help the user determine what other resource (howto, concept, or reference) they should look at next.

.Document Preview
--
Your preview should contain a new section, `2. Hello, World! as an Acme REST service` that includes the contents of `src/main/java/org/acme/MyAcmeApplicaton.java` with the `goodbye` method omitted.
--
...
- We describe how to use `curl` to gest the output of our defined endpoint.

.Document Preview
--
Your preview should contain a new section, `3. Dev Mode: Hello, World!` that shows 3 methods (cli, maven, gradle) of launching Quarkus dev mode, shows codeblocks containing curl console commands and output, and instructions for how to exit dev mode.
--
...
Create a new source file, `src/test/java/org/acme/MyAcmeApplicatonTest.java`, and define `MyAcmeApplicatonTest` as follows:

[source,java]
----
\include::{examples}/acme-serve-http-requests-MyAcmeApplicatonTest.java[tags=*;!goodbye]
----

After saving, the dev console should detect the presence of tests, but it isn't running by default. The bottom of the console screen will display a message indicating that running tests have been paused.

Press `r` to resume testing. You should see the status change as tests are running, and it should finish with a message indicating that 1 test was run and that it was successful.
-----

Just as with the original source file, we're using Asciidoc region tags to exclude a method from the listing (that we will be adding later).

[NOTE]
Console output and commands for the Quarkus dev mode may change with time, so we describe the kind of confirmation they should be seeing, rather than trying to capture or reproduce it.

.Document Preview
--
Your preview should contain a new section, `4. Testing: Hello, World!` that includes the includes the contents of `src/main/java/org/acme/MyAcmeApplicatonTest.java` with the `testGoodbyeEndpoint` method omitted.
--
...
[source,java]
----
\include::{examples}/acme-serve-http-requests-MyAcmeApplicaton.java[tag=goodbye]
----
...
Add the following method to `src/test/java/org/acme/MyAcmeApplicatonTest.java`:

[source,java]
----
\include::{examples}/acme-serve-http-requests-MyAcmeApplicatonTest.java[tag=goodbye]
----
...
`goodbye` from `src/main/java/org/acme/MyAcmeApplicaton.java`, and `testGoodbyeEndpoint` from `src/main/java/org/acme/MyAcmeApplicatonTest.java`.

Some additional things to notice:

- We are again using an include and tag regions, though in this case we're specifying a region to include, rather than a region to exclude.
...
[NOTE]
-----
include::{examples}/acme-serve-http-requests-tutorial.adoctxt[]
-----